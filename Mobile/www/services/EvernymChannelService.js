...
function EvernymChannelService(api) {
  ...
  this.createChannel = function(channel, callbacks) {
    ...
    // logger.log('starting to add channel ' + channel.name , null,
    // 'dataservice.channel', true);
  ...
  this.listMyChannels = function(callbacks) {
    ...
    return api.callAPI('GET', '/channel?relationship=O', undefined, callbacks,
        true);
  ...
  this.listFollowingChannels = function(callbacks) {
    ...
    // /GET /channel?relationship=O for owner and GET /channel?relationship=F
    return api.callAPI('GET', '/channel?relationship=F', undefined, callbacks,
        true);
  ...
  this.followChannel = function(channelid, callbacks) {
    ...
    return api.callAPI('POST', '/channel/' + channelid + '/follower',
  ...
  this.getFollowers = function(channelid, callbacks) {
    ...
    return api.callAPI('GET', '/channel/' + channelid + '/follower', undefined,
        callbacks, true);
  ...
  this.getFollower = function(followerid, callbacks) {
    return api.callAPI('GET', '/follower/' + followerid, undefined, callbacks,
        true);
  };

  this.unfollowChannel = function(channelid, callbacks) {
    return api.callAPI('DELETE', '/channel/' + channelid + '/follower',
        undefined, callbacks, true);
  };

  this.removeFollower = function(channelid, followerid, callbacks) {
    return api.callAPI('DELETE', '/channel/' + channelid + '/follower/'
        + followerid, undefined, callbacks, true);
  };
  ...
  this.provisionalEnroll = function(provisional, callbacks) {
    return api.callAPI('POST', '/account/provisional/enroll', provisional,
        callbacks, true);
  };
