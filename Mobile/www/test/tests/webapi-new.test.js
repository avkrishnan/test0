(function() {
  ...
  asyncTest('LOGOUT', function() {
    $.when(api.sLogout(SCEN1.accessToken)).then(api.CHECK.successNoContent).then(start);
  });
  ...
  asyncTest('LOGIN INVALID PASSWORD', function() {
    ...
    $.when(api.sLogin(invalidLogin)).then(api.CHECK.unauthorized).then(start);
  });
  ...
  function fetchAcctAndCheck(checkFunc) {
    $.when(api.sFetchAccount(SCEN1.accessToken)).then(api.CHECK.success).then(checkFunc);
  }

  asyncTest('FETCH ACCOUNT INFO', function() {
    $.when(
        fetchAcctAndCheck(hlpr.checkEqualC(SCEN1.account, [ 'accountname', 'firstname', 'lastname',
            'nickname' ]))).then(start);
  });
  ...
  asyncTest('CHANGE ACCOUNT NAME', function() {
    ...
    $.when(api.sModifyAccount(SCEN1.accessToken, newAccount)).then(api.CHECK.notImplemented).then(
        start);
  });

  function modifyAcctAndCheck(newAccount, checkFunc) {
    return $.when(api.sModifyAccount(SCEN1.accessToken, newAccount)).then(
        api.CHECK.successNoContent).then(function() {
  ...
  asyncTest('CHANGE ACCOUNT NAME WITH SAME NORMALIZED NAME', function() {
    ...
    modifyAcctAndCheck(newAccount, hlpr.checkEqualC(newAccount)).then(start);
  });
  ...
  asyncTest('GET COMMUNICATION METHODS', function() {
    $.when(api.sFetchComMethods(SCEN1.accessToken)).then(api.CHECK.success).then(function(data) {
  ...
  asyncTest('FETCH CHANNELS WHEN THERE ARE NONE', function() {
    $.when(api.sFetchOwnerChannels(SCEN1.accessToken)).then(api.CHECK.success).then(start);
  });

  asyncTest('CREATE CHANNEL', hlpr.createChannelF(SCEN1, 'channel'));

  asyncTest('FETCH A CHANNEL I OWN', function() {
    $.when(api.sFetchChannel(SCEN1.accessToken, SCEN1.channel.id)).then(api.CHECK.success).then(
        start);
  });
  ...
  asyncTest('MODIFY CHANNEL DESCRIPTION', function() {
    ...
    $.when(api.sModifyChannel(SCEN1.accessToken, chnlId, newChnl)).then(api.CHECK.successNoContent)
        .then(hlpr.fetchChannelAndCheckF(SCEN1, 'channel', newChnl)).then(function() {
  ...
  asyncTest('FETCH ALL CHANNELS I OWN', function() {
    $.when(api.sFetchOwnerChannels(SCEN1.accessToken)).then(api.CHECK.success).then(
        checkChannelsMatch).then(start);
  });
  ...
  function fetchAndCheckChannels(data) {
    return $.when(api.sFetchOwnerChannels(SCEN1.accessToken)).then(api.CHECK.success).then(
        checkOneChannel);
  }
  ...
  function testCreateChannelWithFail(accessToken, checkFuncOvd, chnlOvd) {
    ...
    $.when(api.sCreateChannel(accessToken, channel)).then(
        checkFuncOvd ? checkFuncOvd : api.CHECK.unauthorized).then(start);
  }
