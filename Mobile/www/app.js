var ES = ServiceContext();

ES.evernymService.doAfterDone = function() {
  $.mobile.hidePageLoadingMsg();
};

ES.evernymService.doAfterFail = function(ajaxParams, jqXHR, textStatus,
    errorThrown, details) {
  $.mobile.hidePageLoadingMsg();
...
ko.bindingHandlers.updateListviewOnChange = {
  update : function(element, valueAccessor) {
    ...
    var listview = $(element).parents().andSelf().filter(
        "[data-role='listview']");
    ...
    if (listview) {
      try {
        ...
        // if the list view is not initialized, the above call with throw an
        // exception
...
function inviteFollowers() {
  ...
  if (!channelCount) {
    ...
    localStorage.setItem("currentChannel", JSON.stringify(channelListViewModel
        .channels()[0]));
    $.mobile.changePage("#inviteFollowersView", {
...
function showError(msg) {
  ...
  // TODO: add some code to append the message to the box, instead of just
  // ignoring and throwing it away if there is already
  // an existing error message.

  if (!existingdiv) {

    $(
        "<div id='errordiv' class='ui-loader ui-overlay-shadow ui-body-e ui-corner-all'><h3>"
            + msg
...
function submitFeedback() {
  ...
  function sentFeedback(data) {
    $("#feedbackdiv").fadeOut(
        ...
        function() {
          $(this).remove();
          showMessage("Thanks. Your feedback has been submitted for the page: "
              + viewid + " " + viewname);
        });
  }
  function errorSendingFeedback(data, status, details) {
    ...
    if (loginPageIfBadLogin(details.code)) {
      ...
      showError("Error Sending Feedback: "
          + ((status == 500) ? "Internal Server Error" : details.message));
    }
  }
  var callbacks = {
...
function convDate(d) {
  ...
  var output = months[month] + ' ' + day + ', ' + d.getFullYear() + ', '
...
var loginViewModel = new LoginViewModel(), channelListViewModel = new ChannelListViewModel(), channelsFollowingListViewModel = new ChannelsFollowingListViewModel(), channelViewModel = new ChannelViewModel(),

channelMenuViewModel = new ChannelMenuViewModel(), channelSettingsViewModel = new ChannelSettingsViewModel(), channelBroadcastsViewModel = new ChannelBroadcastsViewModel(), channelNewViewModel = new ChannelNewViewModel(),
...
/* By Devedner */
escalationPlansViewModel = new EscalationPlansViewModel(), escalationPlanSingleViewModel = new EscalationPlanSingleViewModel(), addContactViewModel = new AddContactViewModel(), additionalContactViewModel = new AdditionalContactViewModel(), verifyContactViewModel = new VerifyContactViewModel(),
/* end */

/* By chaman */
tutorialViewModel = new TutorialViewModel(), signupStepFirstViewModel = new SignupStepFirstViewModel(), signupStepSecondViewModel = new SignupStepSecondViewModel(), channelsIOwnViewModel = new ChannelsIOwnViewModel(), helpViewModel = new HelpViewModel(), aboutEvernymViewModel = new AboutEvernymViewModel(), feedbackViewModel = new FeedbackViewModel(), inviteFollowersIIViewModel = new InviteFollowersIIViewModel(), privacyPolicyViewModel = new PrivacyPolicyViewModel(),
/* end */

messageViewModel = new MessageViewModel(), singleMessageViewModel = new SingleMessageViewModel(), unsubscribeModel = new UnsubscribeModel(), selectIconViewModel = new SelectIconViewModel(), followChannelViewModel = new FollowChannelViewModel();

// load the stored state (recent searches)

$.mobile.defaultPageTransition = ""; // "slide";

var models = [ loginViewModel, channelListViewModel,
    channelsFollowingListViewModel, channelViewModel, channelMenuViewModel,
    channelSettingsViewModel, channelBroadcastsViewModel, channelNewViewModel,
    sendMessageViewModel, followersListViewModel, inviteFollowersViewModel,
    followerViewModel, commethodVerificationViewModel, userSettingsModel,
    devSettingsModel, notificationsViewModel, forgotPasswordViewModel,
    resetPasswordViewModel, panelHelpViewModel, messageViewModel,
    singleMessageViewModel, unsubscribeModel, selectIconViewModel,
    followChannelViewModel, escalationPlansViewModel,
    escalationPlanSingleViewModel, addContactViewModel,
 ...
 */
/*
 * function isPhoneGap() { return (cordova || PhoneGap || phonegap) &&
...
function loadAllPages() {
  ...
  for (i = 0, len = models.length; i < len; i += 1) {
    ...
    var promise = $.mobile.loadPage("views/" + page, {
      pageContainer : $('#allpages'),
      allowSamePageTransition : true
    });
...
$(document).on(
    ...
    function(e, a) {
      ...
      $(this).find('#gpaneldots').append(
          $("#globalpaneldots #mypaneldots").clone());
...
$(document).on('pagebeforeshow', '[data-role="page"]', function(e, a) {
  ...
  if (vm && vm.hasfooter && token) {
    // The reason the footer is appended on the pagebeforeshow instead of
    // pagebeforecreate is because it relies on the viewmodel for some
  ...
  /*
   * 
   * //if( panelHelpViewModel.isDirty($(this).attr('id'))){ var panelhtml =
   * $("#globalpanel").find('#mypanel').html();
   * $(this).find('#mypanel').html(panelhtml); $(this).find('#mypanel').panel();
   * 
   * 
  ...
  /*
   * var panel = $(this).find('#mypanel').get(0); if (panel){ myScroll = new
   * iScroll(panel); }
   */
...
/*
 * $(document).on('swiperight', '[data-role="page"]', function(e,a){ if
 * (!openPanel) $(this).find('#mypanel').panel("open");
 * 
 * });
 * 
 */
